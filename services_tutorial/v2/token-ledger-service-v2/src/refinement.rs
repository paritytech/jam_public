...
use token_ledger_state_v2::merkle::Witness;
...
pub struct Payload {
    ...
    pub witness: Option<token_ledger_state_v2::merkle::Witness>,
}
...
pub fn refine_payload(payload: &[u8]) -> (Vec<u8>, usize) {
    ...
    let Some((previous_root, new_root, operations_len, version, _)) = refine_transition(
        payload,
        &mut to_solicit,
...
fn refine_transition(
    ...
    let witness = match version {
        Mode::Extrinsic => {
            ...
            let witness: Witness = match Decode::decode(&mut &extrinsic_data[..]) {
                Ok(t) => t,
                Err(e) => {
                    error!("Failed to decode witness from extrinsic: {:?}", e);
                    return None
                }
            };
    ...
    return Some((previous_root, new_root, operations_len, version, read_size));
}
