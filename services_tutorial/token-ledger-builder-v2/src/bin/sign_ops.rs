...
use token_ledger_common::json::{UnsignedOperationJson, SignedOperationJson, parse_unsigned_operations};
...
fn main() {
    ...
    input.read_to_end(&mut input_vec).expect("Failed to read input file");

    let unsigned_ops = parse_unsigned_operations(input_vec.as_slice()).expect("Failed to parse unsigned operations");
    let mut signed_ops = Vec::new();

    for op in unsigned_ops {
        let signed_op = match op {
            UnsignedOperationJson::Mint { token_id, amount, to_seed } => {
            ...
            UnsignedOperationJson::Transfer { token_id, amount, from_seed, to_seed } => {
